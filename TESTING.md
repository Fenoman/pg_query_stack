...
### 013_query_length_limit.sql - Полный текст запроса без усечения
   - Проверяет что pg_query_stack возвращает полный sourceText без обрезки
   - Важно для аудита длинных DDL-запросов и больших миграций
   - Использует CTE с большой константной строкой
   - Ожидаемый результат: длина query_text равна полной длине исходного SQL
