-- Тест 017: Глубокая вложенность plpgsql функций
--
-- Walker SRF использует snapshot-once + lazy pstrdup: ring хранит только
-- указатели qd, текст копируется один раз при вызове pg_query_stack().
-- Тест проверяет, что при глубине стека 5+ все фреймы возвращают корректный
-- query_text и нет ни одного "пустого" или повреждённого слота.
--
...
-- условия выполнения в Citus, plpgsql и TimescaleDB.
--
